...
from orm_import.utils import object_mapper, get_object_for_tag
@validate_call
...
class deleter(BaseModel):
    ...
    def delete(self,session, the_object, messanger, force=False):
        try:
            ...
            to_be_deleted=list(chain( *[v for v in to_delete.values()]))
            ...
            if len(self.doubly_converged)>0 and not force:
                ...
                if not force:
                    messanger.add_message(f"Found {sum([ len(v) for v in to_delete.values() ])} items to delete provide force keyword to remove them")
                else:
                ...
                messanger.add_message(f"Found no entries to delete from table {the_object.__name__}")
            return messanger
