...
object_mapper.update({
    ...
    NAME_CONF   : Conformation,
    NAME_GROUP  : Group,
    NAME_WFN_DAT : Wave_Function_Run_Files,
    NAME_DISPOL: Distributed_Polarisabilities,
})

for k,v in object_mapper.items():
    found=[ kk for kk,v2 in names.items() if kk.lower() == k.lower() ]
    assert len(found)<=1, f"Duplicate key found in object mapper: {k} in {found}"
    if len(found)==1:
        names[found[0]].append(v.__name__)
    else:
        names[k] = [v.__name__]
...
def get_object_for_tag(tag):
    try:
        ...
        mapper=update(mapper, Wave_Function_Run_Files)
        mapper=update(mapper, Molecular_Polarizability)
        mapper=update(mapper, Molecular_Polarizability_Run_Files)
        ...
        for k,v in object_mapper.items():
            mapper=update(mapper, v, k.lower())
